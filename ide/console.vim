...
let s:console_id = -1
let s:console_script = fnameescape(join([expand('<sfile>:h:h:h'), 'scripts', 'console.sh'], '/'))
...
function! s:CreateBuffer() abort
    execute 'enew'
    let s:console_id = cmake#job#TermStart(s:console_script, function('s:CMakeConsoleCb'))
...
"
function! ConsoleOpen(clear) abort
...
"
"function! ConsoleSetCmdId(id) abort
    "if a:id ==# 'generate'
        "let s:cmd_id = a:id
